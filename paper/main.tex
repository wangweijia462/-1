...
\begin{equation}
  u_{od}^k = \frac{V_d}{c_{od}\cdot\epsilon_{od}^k},
  \label{eq:random_utility}
\end{equation}
其中$V_d$为城市$d$的间接效用（方程\ref{eq:welfare}），
$c_{od}>0$为双边迁移成本，
$\epsilon_{od}^k$为刻画个体特质（偏好、社会网络等）的随机变量，
满足Fréchet分布$\Pr(\epsilon_{od}^k\leq z)=\exp(-z^{-\kappa})$，
$\kappa>0$为集中度（迁移弹性）参数。
利用Fréchet分布的极值性质，
个体$k$选择使效用最大的城市$d$的概率为
\begin{equation}
  m_{od}=\Pr\!\left[\frac{V_d}{c_{od}\epsilon_{od}^k}
    =\max_{d'}\frac{V_{d'}}{c_{od'}\epsilon_{od'}^k}\right]
...
\begin{equation}
  ...
  \label{eq:pareto}
\end{equation}
其中$\lambda$为增长权重，$1-\lambda$为公平权重。
...
防止人口集聚被高房价完全抵消。
两类政策的联合效应具有超加性特征：
单独实施4b或8分别产生0.32\%和0.35\%的GDP增益，
而联合实施的增益（约0.58\%）大于两者之和的线性预测值，
说明两类政策存在正向互补。
...
约为当期降幅的8倍。
若只观察冲击当期GDP变化，将系统性低估房地产下行的中长期增长代价。
...
比较两类冲击的IRF，两点值得关注：
（1）纯财政冲击（$\varepsilon^G$）的GDP峰值降幅更大（$-0.86\%$ vs. $-0.57\%$），
缺少房价下跌带来的住房成本降低正效应；
...
结果如前所述：两类政策存在正向互补，GDP联合增益（0.58\%）超过线性加总（0.67\%→实为超加）。
这一结果说明，忽视迁移壁垒变化的静态模型会低估土地制度改革的实际效果，
在城镇化深化阶段尤为如此。
