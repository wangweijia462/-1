function [Wel,DWel,DrealGDP,DGini_GDP,DTheil_Wel] = spatial_equilibrium_model_Ry_Rh_wel(psi_sell_ry,psi_buy_ry,psi_sell_rh,psi_buy_rh,prov_code,m_od_17,TS_2017,r_o,r_2017,Ry_2017,Rh_2017,tar_ry_2017,tar_rh_2017,A_2017,V_2017,c_od_2017,L_2017,H_2017,Lh_2017,Ph_2017,A_tar_2017,N_2017,Pry1,N,alpha,eta,kappa,sigma,rho,Pl_2017,Pry,Prh)
...
for i = 1:length(unique_prov_ry)
    ...
    F_values_ry(abs(idx_ry_o),:) = r_o_values_ry(abs(idx_ry_o),:).*TS_2017_values_ry(abs(idx_ry_o),:);
...
for i = 1:length(unique_prov_rh)
    ...
    F_values_rh(abs(idx_rh_o),:) = r_o_values_rh(abs(idx_rh_o),:).*psi_sell_values_rh(abs(idx_rh_o),:).*TS_2017_values_rh(abs(idx_rh_o),:);
