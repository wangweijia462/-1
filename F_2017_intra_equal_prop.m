%% counterfactual
% 1.1仅有几个省份的地级市被允许调剂建设用地指标【生产性用地-等比例分配】
% 限定在“三区三州”及其他深度贫困县
close all
clear all
addpath(genpath(pwd));
...
for i = valid_groups
    ...
    proportion(abs(idx_d),:) = Ry_2017_values(abs(idx_d),:) ./ sum(Ry_2017_values(abs(idx_d),:));
    allocated_amounts(abs(idx_d),:) = TS_total_2017 .* proportion(abs(idx_d),:);
    Ry_values = Ry(idx); 
    F_values = F(idx);
...
% 1.2所有省份的地级市被允许省内调剂建设用地指标【生产性用地-等比例分配】
close all
clear all
addpath(genpath(pwd));
initial_eqm_2017
...
% 1.3仅有几个省份的地级市被允许调剂建设用地指标【居住性用地-等比例分配】
% 限定在“三区三州”及其他深度贫困县
close all
clear all
addpath(genpath(pwd));
...
for i = valid_groups
    ...
    proportion(abs(idx_d),:) = Rh_2017_values(abs(idx_d),:) ./ sum(Rh_2017_values(abs(idx_d),:));
    allocated_amounts(abs(idx_d),:) = TS_total_2017 .* proportion(abs(idx_d),:);
    Rh_values = Rh(idx); 
    F_values = F(idx);
    r_2017_values = r_2017(idx);
...
% 1.4所有省份的地级市被允许省内调剂建设用地指标【居住性用地-等比例分配】
close all
clear all
addpath(genpath(pwd));
initial_eqm_2017
...
% 1.5仅有几个省份的地级市被允许调剂建设用地指标【两类用地-等比例分配】
% 限定在“三区三州”及其他深度贫困县
close all
clear all
addpath(genpath(pwd));
