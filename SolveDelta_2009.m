XX=fsolve(@(XX)SolveCounter_2009(XX,Ry,Rh,tar_ry_2009,tar_rh_2009,A_2009,V_2009,c_od_2009,L_2009,H_2009,Lh_2009,Ph_2009,A_tar_2009,N_2009,F,Pry1,N,alpha,eta,kappa,sigma,rho,Pl_2009,Pry,Prh), ...
...
% (1)产出GDP
Y_2009 = A_2009.*(L_2009.^alpha).*(Ry_2009.^(1-alpha));
Y_2009_new = A_2009_new.*(L_2009_new.^alpha).*(Ry.^(1-alpha));
omega_Y = Y_2009./sum(Y_2009);% weigth of welfare: population weight
drealGDP = (sum(Y_2009_new./Y_2009.*omega_Y)-1)*10;
% (2)产出差距之Gini_GDP 基尼系数
Gini_old_gdp = SolveGini(Y_2009,ones(N,1));
Gini_new_gdp = SolveGini(Y_2009_new ,ones(N,1));
dGini_GDP = (Gini_new_gdp./Gini_old_gdp-1)*10;
% (3)福利Wel
omega_L = N_2009./sum(N_2009); % weigth of welfare: population weight
Welj = (V_2009.*(diag(m_od_09)).^(-1/kappa));
Wel = omega_L'*Welj;
dWelj=(dV_2009.*(diag(m_od_09_new./m_od_09)).^(-1/kappa));
dWel = (omega_L'*dWelj-1)*10;
% (4)福利差距之Theil_Wel 
Theil_old_Wel = SolveTheil(Wel,L_2009);
Welj_new = (V_2009_new.*(diag(m_od_09_new)).^(-1/kappa));
Wel_new = omega_L'*Welj_new;
Theil_new_Wel = SolveTheil(Wel_new,L_2009_new);
